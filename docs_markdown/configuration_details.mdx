...
Readiness checks represent checks that are `boolean` in nature. A result of such checks always presents a `True`/`False` value with some explanation in case of check fails. They can be triggered with a common method: [`CheckFirewall.run_readiness_checks()`](./api/check_firewall.md#checkfirewallrun_readiness_checks). When running this method, a list of readiness checks to run (with an optional configuration) is provided using the `checks_configuration` parameter. This list is defined using the [`ConfigParser`](#utils.md#class-configparser) class [`dialect`](./dialect.mdx). For details, refer to the documentation for this class.
...
Following [`ConfigParser's`](#utils.md#class-configparser) [`dialect`](dialect.mdx), when no configuration is provided, **all** checks are triggered. *Notice* that in this situation checks that require configuration to run will return [`CheckStatus.SKIPPED`](utils.md#class-checkstatus).
...
The [`CheckFirewall.run_readiness_checks()`](./api/check_firewall.md#checkfirewallrun_readiness_checks) take one more parameter: `report_style`. Format of the returned data depends on the value of this parameter. If the value is:
...
- `False` (default): results of executed checks are formatted as `dict`, where keys are check names as passed in `checks_configuration` and values are dictionaries containing two keys:
  ...
  - `state`: a `bool` representation of the [`CheckResult`](#utils.md#class-checkresult) class for a particular check.
  - `reason`:  a `str` representation of the [`CheckResult`](#utils.md#class-checkresult) class for a particular check.
...
- `True`: results are also formatted as `dict` with the keys corresponding to checks names, but values are a string representations of the [`CheckResult`](#utils.md#class-checkresult) class.
...
They can be triggered using a common method: [`CheckFirewall.run_snapshots()`](./api/check_firewall.md#checkfirewallrun_snapshots). When running this method a list of state areas is provided using the `snapshot_config` paramter. This list is specified in [`ConfigParser`](#utils.md#class-configparser) class [`dialect`](./dialect.mdx). For details, refer to the documentation for this class.

Following that [`dialect`](./dialect.mdx), when no list is passed, **all** state snapshots are made.

The output of the [`CheckFirewall.run_snapshots()`](./api/check_firewall.md#checkfirewallrun_snapshots) method is formatted as a dictionary where:

- keys are state areas as passed in `snapshot_config` parameter,
- values are dictionaries with the actual data.

  Each dictionary can have a different structure. The structure depends on the nature of data we want to capture (see sections below for details).
...
Method used: `FirewallProxy.get_arp_table()`.

### `content_version`

Grabs the currently installed Content DB version.

Method used: `CheckFirewall.get_content_db_version()`.
...
Method used: `CheckFirewall.get_ip_sec_tunnels()`.
...
Method used: `FirewallProxy.get_licenses()`.
...
Method used: `FirewallProxy.get_nics()`.
...
Method used: `FirewallProxy.get_routes()`.
...
Method used: `FirewallProxy.get_session_stats()`.

## Reports

The reporting part is actually the result of comparison of two
snapshots. It is advised to run reports using the common method as some
...
(`SnapshotCompare.get_diff_and_threshold`). It produces the standardized dictionary.
...
snapshots. See the `nics_snapshot`
...
`FirewallProxy.get_session_stats` method
...
`SnapshotCompare.get_count_change_percentage()` method documentation.
