...
Verifies each certificates' key size against a minimum value.

**Method**: [`CheckFirewall.check_ssl_cert_key_size()`](/panos/docs/panos-upgrade-assurance/api/check_firewall#checkfirewallcheck_ssl_cert_key_size)

**Configuration parameters**

parameter | description
--- | ---
`minimum_key_size` | (optional) minimum allowable key size (in bytes)
